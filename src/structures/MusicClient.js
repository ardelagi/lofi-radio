...
class MusicBot extends Client {
  constructor() {
    ...
    this.manager; // Kazagumo node manager
  ...
  _loadSlashCommands() {
    ...
    readdirSync("./src/slashCommands").forEach((dir) => {
      const slashCommandFile = readdirSync(`./src/slashCommands/${dir}/`).filter(f => f.endsWith(".js"));
      for (const file of slashCommandFile) {
        const slashCommand = require(`../slashCommands/${dir}/${file}`);
        if (!slashCommand.name) return console.error(`slashCommandNameError: ${file} application command name is required.`);
        if (!slashCommand.description) return console.error(`slashCommandDescriptionError: ${file} application command description is required.`);
  ...
  async _connectMongodb() {
    ...
    mongoose.connection.on("connected", () => this.logger.log("[DB] DATABASE CONNECTED", "ready"));
    mongoose.connection.on("err", (err) => this.logger.log(`[DB]Mongoose connection error: \n ${err.stack}`, "error"));
    mongoose.connection.on("disconnected", () => this.logger.log("[DB]Mongoose disconnected", "error"));
  }
